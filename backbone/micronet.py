...
class DYMicroBlock(nn.Module):
    def __init__(self, inp, oup, flag, kernel_size=3, stride=1, ch_exp=(2, 2), ch_per_group=4, groups_1x1=(1, 1), depthsep=True, shuffle=False, pointwise='fft', activation_cfg=None):
        ...
        def micro_block_c(need):
                ...
                self.layers = nn.Sequential(
                    ...
                    activation.get_act_layer(
                        ...
                        g = gs1,
                        expansion = False
                    ) if y1 > 0 and need == 1 else nn.ReLU6(inplace=True),
                    ChannelShuffle(gs1[1]) if shuffle else nn.Sequential(),
                    ...
                    ChannelShuffle(hidden_dim2//4) if shuffle and y1!=0 and y2 !=0 else nn.Sequential() if y1==0 and y2==0 else ChannelShuffle2(hidden_dim2//2),
        ...
        if flag =='A':
            micro_block_a(self.need)
        elif flag == 'B':
            micro_block_b(self.need)
        elif flag == 'C':
            micro_block_c(self.need)
...
class MicroNet(nn.Module):
    def __init__(self, cfg, input_size=224, num_classes=1000, teacher=False):
        ...
        self.cfgs,block_num = microcfg.get_micronet_config(mode)
