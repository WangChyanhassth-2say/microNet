...
class DYShiftMax(nn.Module):
	...
	# try adding alphas & lambdas
        self.register_buffer("lambdas", torch.Tensor([0.5] * 2).float())
        self.register_buffer("alphas", torch.Tensor([1.0] + [0.0]).float())
       ...
       # print('init-a: {}, init-b: {}'.format(init_a, init_b))
        ...
        self.fc1 = nn.Linear(oup, squeeze)
        self.fc2 = nn.Linear(squeeze, oup*self.exp)
    ...
    def forward(self, x):
       ...
       # print('theta_size = ',self.get_relu_coefs(x).size())
       # print('channels = ',channels)
        try:
            relu_coefs = self.get_relu_coefs(x).view(-1, channels, 2, 2)
            relu_coefs = relu_coefs.permute(0, 1, 3, 2) * self.lambdas
            ...
            if self.exp == 4:
                ...
                CG = self.gc
                out = x.expand(2, -1, -1, -1, -1)
                index = ((torch.arange(channels).cuda().expand(2, -1).permute(1, 0) + torch.arange(2).cuda() * CG) % channels).permute((1, 0)).expand(x.shape[0], x.shape[2], x.shape[3], -1, -1).permute(3, 0, 4, 1, 2)
                out = (torch.gather(out, dim=2, index=index).permute(3, 4, 1, 2, 0).unsqueeze(dim=-1) * relu_coefs).sum(dim=-2)
                out = torch.max(out,dim=-1)[0].permute(2, 3, 0, 1)
